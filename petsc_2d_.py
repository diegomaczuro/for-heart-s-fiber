...
def read_tensor(filename):
    ...
    D = np.zeros((n*n,3,3))
    R = np.zeros((n*n,3,3))
    D[:,0,0] = np.reshape(e1[:,:,slic],n*n)*koef
    D[:,1,1] = np.reshape(e2[:,:,slic],n*n)*koef
    D[:,2,2] = np.reshape(e3[:,:,slic],n*n)*koef
    R[:,0,0] = np.reshape(v11[:,:,slic],n*n)
    R[:,0,1] = np.reshape(v12[:,:,slic],n*n)
    R[:,0,2] = np.reshape(v13[:,:,slic],n*n)
    R[:,1,0] = np.reshape(v21[:,:,slic],n*n)
    R[:,1,1] = np.reshape(v22[:,:,slic],n*n)
    R[:,1,2] = np.reshape(v23[:,:,slic],n*n)
    R[:,2,0] = np.reshape(v31[:,:,slic],n*n)
    R[:,2,1] = np.reshape(v32[:,:,slic],n*n)
    R[:,2,2] = np.reshape(v33[:,:,slic],n*n)
